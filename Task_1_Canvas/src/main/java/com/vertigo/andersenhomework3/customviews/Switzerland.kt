...
class Switzerland(context: Context, attr: AttributeSet? = null): View(context, attr) {
    ...
    private fun drawSecond(canvas: Canvas?) {
        canvas?.apply {
            val leftSide = width - (width / 9) * 4.toFloat()
            val rightSide = width - (width / 9) * 5.toFloat()
            val topSide = 70f
            val botSide = height.toFloat() - 70
    ...
    private fun drawThird(canvas: Canvas?) {
        canvas?.apply {
            val leftSide = 70f
            val rightSide = width.toFloat() - 70
